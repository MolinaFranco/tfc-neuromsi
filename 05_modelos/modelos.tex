...
Conviene distinguir el papel de las Ecuaciones~\ref{eq:latent_to_membrane} y \ref{eq:ssn_dynamics}, que a primera vista parecen definir dos veces la misma variable. La Ecuación~\ref{eq:latent_to_membrane} no describe ninguna dinámica: fija el código, es decir, qué valor de la variable latente $y_i$ representa cada potencial de membrana, y de ella se derivan los momentos objetivo que la red debe reproducir. Quien determina la evolución temporal de $u_i$ es exclusivamente la Ecuación~\ref{eq:ssn_dynamics}. El entrenamiento consiste justamente en ajustar los parámetros de esa dinámica para que la distribución estacionaria de los $u_i$ que ella genera coincida con la que induce la Ecuación~\ref{eq:latent_to_membrane} al muestrear $y_i$ de la posterior del \acrshort{GSM}.
...
El costo del entrenamiento no proviene de la cantidad de parámetros, que es mínima para los estándares del aprendizaje automático, sino de lo que cuesta evaluar el gradiente de cada uno. La función objetivo no se calcula sobre una respuesta determinista: exige simular la red estocástica completa durante cientos de milisegundos, para los cinco estímulos y para decenas de trials por estímulo, estimar a partir de esas trayectorias los momentos de la distribución de respuestas, y recién entonces propagar el gradiente hacia atrás a través de todo ese proceso temporal. Optimizar 15 parámetros así resulta considerablemente más caro que ajustar miles de pesos de una red feedforward sobre un lote de datos.
...
con $\alpha = 1, \ldots, 5$ y $z^{(\alpha)} \in \{0, 0.125, 0.25, 0.5, 1.0\}$. El contenido $\bar{\mathbf{y}}$ es una función gaussiana de $27^\circ$ de ancho centrada en $0^\circ$ (una única orientación dominante). Gracias a la parametrización rotacionalmente invariante, entrenar en un estímulo equivale a entrenar en todas sus posibles rotaciones.

\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.85\textwidth]{05_modelos/figures/training_stimuli_contrast.png}
    \caption{
        Conjunto de entrenamiento del modelo de Echeveste. Los cinco estímulos comparten el mismo contenido $\bar{\mathbf{y}}$ (gaussiana de $27^\circ$ centrada en $0^\circ$) 
...
El cuarto término es un costo de ``lentitud'' (\textit{slowness}) que penaliza la autocorrelación temporal de las respuestas. La motivación es que una red que muestrea la posterior recorre el espacio de estados a lo largo del tiempo, y si su actividad cambia con demasiada lentitud las muestras consecutivas se parecen entre sí: en una ventana de tiempo fija se obtienen muchas muestras redundantes en lugar de muchas muestras informativas, y la estimación de la posterior converge más despacio. Penalizar la autocorrelación empuja a la red a decorrelacionarse rápido, es decir, a explorar más en el mismo tiempo:
...
donde $\mathbf{C}^{(\alpha)}(\tau) = \text{corr}(\mathbf{u}(t), \mathbf{u}(t + \tau))$ es la matriz de correlación entre la actividad en dos instantes separados por un retardo $\tau$. Tomar su diagonal deja, para cada neurona, la correlación consigo misma a ese retardo, de modo que el término penaliza únicamente la memoria temporal de cada unidad y no las correlaciones entre neuronas, que son las que codifican la covarianza de la posterior y deben preservarse. La norma de Frobenius al cuadrado suma esas autocorrelaciones sobre toda la población y la integral las acumula sobre los retardos hasta $\tau_{\max}$: el costo es mínimo cuando la autocorrelación de cada neurona decae rápidamente a cero.
...
Otras limitaciones incluyen la arquitectura simplificada de hipercolumna (topología de anillo) que limita la generalización a estímulos más complejos y la ausencia de mecanismos de plasticidad sináptica (en simulación) o aprendizaje continuo. Es importante notar que, si bien la red exhibe una rica dinámica temporal gracias a sus conexiones recurrentes (donde la actividad presente depende de estados anteriores), esto no implica plasticidad en el sentido biológico de reconfiguración sináptica. En el modelo, los pesos sinápticos se determinan mediante un algoritmo de optimización externo (como ADAM o L-BFGS-B) antes de la ejecución; una vez que la red comienza a realizar inferencia, estos pesos permanecen fijos. Esto no significa que la red responda siempre lo mismo: cada estímulo ingresa por la vía feedforward $h_i(t)$ y produce una distribución de respuestas distinta, e incluso generaliza a estímulos que no formaron parte del entrenamiento. Lo que queda congelado es el modelo interno del entorno que la conectividad encarna, de manera que la red no puede reajustarlo si la estadística de los estímulos cambia. En el cerebro ese reajuste correspondería a la plasticidad sináptica, que este modelo no aborda. Finalmente, la aproximación del contraste con su valor verdadero evita abordar el problema de la inferencia jerárquica completa.
