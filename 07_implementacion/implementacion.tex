...
La matriz de conectividad $\mathbf{W}$ se construye a partir de solo 8 parámetros según la Ecuación~\ref{eq:weight_parametrization}. Esta parametrización reducida constituye una decisión de diseño fundamental del modelo. Con 100 neuronas en la red (50 excitatorias y 50 inhibitorias), una matriz de conectividad sin restricciones contendría 10.000 pesos sinápticos independientes. Diez mil parámetros no son muchos para los estándares actuales del aprendizaje automático, y no es el volumen en sí lo que vuelve inviable esa alternativa. El problema es que el conjunto de entrenamiento consta de apenas cinco estímulos: ajustar 10.000 pesos contra cinco condiciones deja el problema masivamente subdeterminado, con infinitas matrices capaces de reproducir esos momentos objetivo y ninguna razón para que alguna generalice. A esto se suma que cada evaluación del gradiente exige simular la red estocástica completa y retropropagar a través del tiempo, con lo cual el costo por parámetro es muy superior al de una red feedforward convencional. La parametrización circulante resuelve ambas cosas a la vez: además de reducir la búsqueda a 8 parámetros, impone por construcción la simetría rotacional del \acrshort{GSM} y la separación de poblaciones excitatorias e inhibitorias, de modo que cualquier solución encontrada respeta la estructura del problema en lugar de tener que descubrirla a partir de cinco ejemplos.
...
La implementación soporta dos modos de simulación que corresponden a diferentes propósitos experimentales del trabajo original. Conviene aclarar de entrada que ambos modos describen únicamente cómo se presenta el estímulo mientras se simula una red ya entrenada: ninguno de los dos ajusta parámetros. El entrenamiento (Sección~\ref{sec:impl_entrenamiento}) es un proceso separado, y las simulaciones que ejecuta internamente para evaluar la función de costo corresponden siempre a la condición de fase única.
...
def _update_noise(self, eta_old, L, dt, tau_n):
    ...
    eta(t+dt) = eta(t)*(1 - dt/tau_n) + sqrt(2*dt/tau_n)*L*xi
    where xi ~ N(0,I) and L is the Cholesky factor of Sigma_eta.
    """
    eps_1 = 1.0 - dt / tau_n
...
El módulo \texttt{data} dentro del paquete \texttt{echeveste} fue implementado como una infraestructura completamente nueva para gestionar el acceso a datos pre-entrenados y configuraciones de modelos. Este diseño responde a la necesidad de separar claramente la lógica de carga de datos de la lógica de simulación, siguiendo el principio de responsabilidad única. Vale aclarar por qué los cargadores son clases auxiliares y no modelos del framework: en Scikit-NeuroMSI una clase de modelo es la que hereda de \texttt{SKNMSIMethodABC}, implementa \texttt{run()} y devuelve un \texttt{NDResult}, es decir, la que \textit{ejecuta} un experimento. Los cargadores no simulan nada, solo proveen los arreglos con los que se configura un modelo, y mantenerlos afuera de esa jerarquía permite que los mismos datos pre-entrenados alimenten a \texttt{Echeveste2020}, a los scripts de validación o a cualquier modelo futuro sin arrastrar la interfaz de simulación.
...
\midrule
\textbf{Conectividad (optimizados)} 
...
\midrule
\textbf{Covarianza ruido (optimizados)} 
